#!/usr/bin/env python3
"""Prepare the remote runtime used by the agreements export DAG."""
...
DEFAULT_BASE_DIR = "/srv/mobilities_transform/agreements_export"
...
def ensure_runtime(args: argparse.Namespace) -> dict[str, Any]:
    ...
    token_path = (
        Path(args.token_path).expanduser().resolve()
        if args.token_path
        else Path(DEFAULT_TOKEN_PATH)
    )
    ...
    export_script = (
        ...
        else base_dir / "export_agreements_observatory.py"
    )
    setup_script = base_dir / "setup_agreements_export.py"
    ...
    env_content = (
        ...
        f"AGREEMENTS_RAW_DB={shell_quote_env_value(args.db)}\n"
        f"AGREEMENTS_RAW_COLLECTION={shell_quote_env_value(args.collection)}\n"
        f"AGREEMENTS_RAW_SOURCE_TYPE={shell_quote_env_value(args.source_type)}\n"
        f"AGREEMENTS_EXPORT_SPREADSHEET_ID={shell_quote_env_value(args.spreadsheet_id)}\n"
        f"AGREEMENTS_EXPORT_SHEET_NAME={shell_quote_env_value(args.sheet_name)}\n"
    )
    if write_text_file(
        env_path,
        env_content,
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Prepare remote runtime for agreements export.")
    parser.add_argument("--base-dir", default=os.getenv("AGREEMENTS_EXPORT_BASE_DIR", DEFAULT_BASE_DIR))
    ...
    parser.add_argument("--db", default=os.getenv("AGREEMENTS_RAW_DB", DEFAULT_DB))
    parser.add_argument("--collection", default=os.getenv("AGREEMENTS_RAW_COLLECTION", DEFAULT_COLLECTION))
    parser.add_argument("--source-type", default=os.getenv("AGREEMENTS_RAW_SOURCE_TYPE", DEFAULT_SOURCE_TYPE))
    parser.add_argument(
        "--spreadsheet-id",
